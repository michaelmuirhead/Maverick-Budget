...
function exportCSV(entries, name) {
  ...
  entries.forEach(e => { const c = getCats(window.__MV__?.customCategories).find(x => x.id === e.category); rows.push([e.date, e.label, c?.label || e.category, e.type, e.type === "income" ? e.amount : -e.amount, (e.tags || []).join(";")]); });
...
function parseCSV(text) {
  ...
  return lines.slice(1).map(r => { const amt = Math.abs(parseFloat(r[4]) || 0); const type = parseFloat(r[4]) >= 0 ? "income" : "expense"; const cl = (r[2]||"").toLowerCase(); const cat = getCats(window.__MV__?.customCategories).find(c => c.label.toLowerCase() === cl) ||{id:"other",label:"Other",icon:"•",color:"#6b7280"}; const tags = r[5] ? r[5].split(";").filter(Boolean) : []; return { label: r[1] || "(imported)", category: type === "income" ? "income" : cat.id, type, amount: amt, date: r[0] || todayStr(), tags }; }).filter(e => e.amount > 0);
}
...
function YearInReview({ entries }) {
  ...
  const topCats = getCats(window.__MV__?.customCategories).filter(c => c.id !== "income").map(c => ({ ...c, total: yearEntries.filter(e => e.category === c.id && e.type === "expense").reduce((s, e) => s + e.amount, 0) })).filter(c => c.total > 0).sort((a, b) => b.total - a.total).slice(0, 3);
...
function RecurringPanel({ nodeId, recurrings, onAdd, onUpdate, onRemove, onAddEntry }) {
  ...
  const RR = ({ r }) => { const cat = getCats(window.__MV__?.customCategories).find(c => c.id === r.category)||{id:"other",label:"Other",icon:"•",color:"#6b7280"}; const freq = FREQ.find(f => f.id === r.frequency); const isE = editingId === r.id; const [ef, setEf] = useState({ label: r.label, amount: String(r.amount), category: r.category, type: r.type, frequency: r.frequency });
...
function EntryRow({ entry, runningBalance, onUpdate, onRemove, onDuplicate, isEditing, onStartEdit, onStopEdit }) {
  const cat = getCats(window.__MV__?.customCategories).find(c => c.id === entry.category)||{id:"other",label:"Other",icon:"•",color:"#6b7280"}; const isInc = entry.type === "income";
...
function DonutChart({ entries }) {
  ...
  const byC = getCats(window.__MV__?.customCategories).filter(c => c.id !== "income").map(c => ({...c, total: expenses.filter(e => e.category === c.id).reduce((s, e) => s + e.amount, 0)})).filter(c => c.total > 0).sort((a, b) => b.total - a.total);
...
function NodePage({ node, parentName, nodes, entries, recurrings, limits, customCategories, onBack, onNavigate, addNode, updateNode, removeNode, reorderNodes, addEntry, updateEntry, removeEntry, addRecurring, updateRecurring, removeRecurring, setLimit, removeLimit, addCategory, removeCategory, getDesc }) {
  ...
  const filtered = search ? directEntries.filter(e => e.label.toLowerCase().includes(search.toLowerCase()) || (getCats(window.__MV__?.customCategories).find(c => c.id === e.category)?.label||"").toLowerCase().includes(search.toLowerCase()) || (e.tags||[]).some(t => t.includes(search.toLowerCase()))) : directEntries;
